...
export default function EquipasPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      lista.forEach((eq: EquipaSlot) => { ns[eq.id] = eq.nome ?? '' })
      setNomes(ns)
    })
  }, [params.id, router])
                ...
                {editando === e.id ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`font-semibold ${e.nome ? 'text-white' : ''}`}
                            style={e.nome ? {} : { color: 'var(--muted)' }}>
                        {e.nome || 'Por definir...'}
                      </span>
                      {e.confirmada && <span className="text-xs" style={{ color: '#4ade80' }}>✓</span>}
                    </div>
